...
class MergeOpportunities
	...
	def initialize(driver) 
        ...
        sObjectRecordsJson = File.read("E:/Projects/Rspec Assignment/SF-QA-Automation-amol/src/testData/testRecords.json")
    	@sObjectRecords= JSON.parse(sObjectRecordsJson)
    	#@sfBulk = Salesforce.login(@mapCredentials['username2'],@mapCredentials['password2'],true)
		#@driver.get "https://test.salesforce.com/login.jsp?pw=#{@mapCredentials['password2']}&un=#{@mapCredentials['username2']}"
	...
	def createTestData(isAllAccess,userNumber)
		userName = @mapCredentials["username#{userNumber}"]
		password = @mapCredentials["password#{userNumber}"]
		@sfBulk = Salesforce.login(@mapCredentials["username#{userNumber}"],@mapCredentials["password#{userNumber}"],true)
...
#delete created records
	...
	def numberOfOpportunitiesInSalesforce(userNumber,pattern=nil)
			user = "@mapCredentials['username" + userNumber + "']"
	  		count = 0
	  		opportunityIds = nil
	  		profile = Salesforce.getRecords(@sfBulk,"User","select Name,Profile.Name from User where Username = '#{user}'")
	  		if(profile.result.records[0]['Profile.Name'] == 'WeWork System Administrator' || profile.result.records[0]['Profile.Name'] == 'WeWork Support and Operations') then
	        	if pattern != nil then
	...
	def isOpportunityMerged(id)
		...
		locations_Interested__c  = opportunity.result.records[0].fetch("Locations_Interested__c")
		locations = locations_Interested__c.chomp(';').split(";")
		...
		return opportunity.result.records[0].fetch("Merged_Opprtunity__c") ,opportunity.result.records[0].fetch("StageName"),locations.length,numOfTours
	end
