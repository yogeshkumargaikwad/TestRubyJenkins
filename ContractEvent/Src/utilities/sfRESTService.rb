...
class SfRESTService
  ...
  def self.loginRequest
    credentails = YAML.load_file(File.expand_path('',Dir.pwd)+'/credentials.yaml')
    #puts "grant type"
    #puts credentails['Staging']['grant_type']
    data = {"grant_type"=>credentails['Staging']['grant_type'],"client_id"=>credentails['Staging']['client_id'],"client_secret"=>credentails['Staging']['client_secret'], "username"=>credentails['Staging']['username'],"password"=>"#{credentails['Staging']['password']}"}
