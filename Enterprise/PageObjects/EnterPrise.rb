...
=end
	...
	def setMoveInDate(date)
		EnziUIUtility.wait(@driver,:id,'Term__c',@timeSetting['Wait']['Environment']['Lightening']['Min'])
	...
	def navigateToCreateOpportunity()
		EnziUIUtility.wait(@driver,:class,"globalCreateTrigger",@timeSetting['Wait']['Environment']['Lightening']['Min'])
	...
	def createNewOrganization(orgName, noOfEmployees = nil)
		EnziUIUtility.wait(@driver,:id,'GlobalActionManager:New_Organization',@timeSetting['Wait']['Environment']['Lightening']['Min'])
		EnziUIUtility.clickElement(@driver,:id,"GlobalActionManager:New_Organization")
		EnziUIUtility.wait(@driver,:id,'Name',@timeSetting['Wait']['Environment']['Lightening']['Min'])
	...
	def createNewContact(accountName, contactName, emailId)
		...
		EnziUIUtility.wait(@driver,:id,'actionManager:New_Decision_Maker',@timeSetting['Wait']['Environment']['Lightening']['Min'])
		sleep(5)
	...
	def buttonEnabled?(buttonId)
		EnziUIUtility.wait(@driver,:id,"#{buttonId}",@timeSetting['Wait']['Environment']['Lightening']['Min'])
	...
	def navigateToAccountDetails(tab_name)
		...
		EnziUIUtility.wait(@driver,nil,nil,@timeSetting['Sleep']['Environment']['Lightening']['Min'])
	...
	def switchToLightening()
		if !(driver.current_url().include? "lightning")
			...
			EnziUIUtility.wait(driver,:id,"userNav-arrow",@timeSetting['Wait']['Environment']['Classic']['Min'])
	...
	def switchToClassic()
		if (@driver.current_url().include? "lightning")
			...
			EnziUIUtility.wait(@driver,:class,"oneUserProfileCardTrigger",@timeSetting['Wait']['Environment']['Lightening']['Min'])
			@driver.find_element(:class, "oneUserProfileCardTrigger").click
			EnziUIUtility.wait(@driver,:class,"profile-card-footer",@timeSetting['Wait']['Environment']['Lightening']['Min'])
...
=end
	...
	def createOpportunity(opportunityRole, oppRoleContact, buildingName, accountName, noOfFTE = nil, noOfDeskInterestedIn = nil, monthlyBudget = nil, location = nil, officeFormat = nil, moveInDate = nil, sqFeet = nil, term = nil, useCase = nil)
		...
		if(moveInDate != nil)
			EnziUIUtility.wait(@driver,:id,'Move_In_Date__c',@timeSetting['Wait']['Environment']['Lightening']['Min'])
