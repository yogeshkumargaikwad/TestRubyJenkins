class EnziUIUtility
  ...
  #e.g.:   selectChildByText(driver,:id,"parentElementId",:tag_name,"childIdentifier","textToselect" )
  def self.selectChildByText(driver,parentfindBy,parentElementId,childFindBy,childIdentifier,textToselect )
    #Get the select element
    puts "In select child by text"
    childText = ""
    parent = driver.find_element(parentfindBy,parentElementId)
    #Get all the options for this element
    all_child = parent.find_elements(childFindBy, childIdentifier)
    #select the options
  ...
  def self.switchToWindow(driver,titleName)
    ...
    self.wait(driver,nil,nil,5)#Get all currnt open window
